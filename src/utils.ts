...
import { CLASS_ID, Prov } from './types';

// Хелперы
export function pick<T>(arr: readonly T[]): T { return arr[Math.floor(Math.random() * arr.length)]; }
export function rand(min: number, max: number) { return min + Math.floor(Math.random() * (max - min + 1)); }
export async function ensureDir(d: string) { await fs.mkdir(d, { recursive: true }); }

export async function safeGoto(page: Page, url: string, timeout: number) {
  try { await page.goto(url, { waitUntil: 'domcontentloaded', timeout }); } catch {}
}
